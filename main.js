...
(async function () {
  ...
  const NEW_DAYS = 7;
  const LOW_STOCK = 3;
  const WISHLIST_KEY = 'bobbyesquire_wishlist';
  let items = [];
  let settings = {};
  let suspended = false;
  ...
  const ANALYTICS_KEY = 'bobbyesquire_analytics';
  ...
  function enquireBody(item, soldOut, selectedSize) {
    ...
    return soldOut
      ? `Hi Bobby Esquire! I saw *${item.name}* is sold out. Will it be back in stock? I'd love to reserve one.`
      : `Hi Bobby Esquire! I'd like to enquire about *${item.name}*${sizePart}${pricePart} from your catalog.`;
  }

  function whatsappLink(item, soldOut, selectedSize) {
    const phone = settings.whatsappNumber || '254702712222';
  ...
  // Group a size token for the size dropdown (menswear: letters + waist + shoe).
  ...
  document.getElementById('wishlistEnquireAll')?.addEventListener('click', e => {
    ...
    const msg = `Hi Bobby Esquire! I'd like to enquire about these saved items:\n\n${lines.join('\n')}\n\nAre they available?`;
  ...
  function showSuspended() {
    ...
    const css = ('@keyframes beSusFade{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}'
      + '#suspendedOverlay{position:fixed;inset:0;z-index:99999;background:radial-gradient(ellipse at top,#2a2e68 0%,#15173a 70%);color:#eef0fb;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:40px 24px;font-family:Inter,system-ui,-apple-system,sans-serif;animation:beSusFade 0.65s ease both;}'
      ...
      + '#suspendedOverlay .be-body{font-size:16px;max-width:460px;line-height:1.65;opacity:0.82;margin:0 0 14px;}'
      + '#suspendedOverlay .be-offer{font-size:16px;max-width:460px;line-height:1.6;margin:0 0 30px;color:#c3c7f2;}'
      + '#suspendedOverlay .be-offer b{color:#fff;font-weight:700;}'
    ...
    const clientBody = (
      '<h1 class="be-head">This website is temporarily offline</h1>'
      + '<p class="be-body">We are back shortly. '
      + (igLink ? 'In the meantime you can see our latest stock and order on Instagram.' : 'Please check back soon.')
      + '</p>'
      + (igLink ? '<a class="be-ig" href="' + igLink + '" target="_blank" rel="noopener">' + IG_SVG + ' See us on Instagram</a>' : '')
    );
    // Prospect copy: pitch the one-off win-back to the shop owner.
    const prospectBody = (
      '<h1 class="be-head">This shop is paused</h1>'
      + '<p class="be-body">Not ready for a monthly plan? You don\'t need one.</p>'
      + '<p class="be-offer">Now you can <b>own this shop outright for a one-time Ksh 20,000</b>, no monthly fees. New stock you post on Instagram pulls straight into your shop. Buyers can filter by category and size to find what they want fast, then order on WhatsApp.</p>'
      + '<a class="be-ig" href="' + waLink + '" target="_blank" rel="noopener">' + WA_SVG + ' Bring my shop back</a>'
    );
