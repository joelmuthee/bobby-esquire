...
const SHOP_URL = 'https://bobbyesquire.essenceautomations.com'; // public storefront — used in WhatsApp messages to clients
...
function checkAuth() {
  if (sessionStorage.getItem('bobbyesquire_auth') === '1') {
...
async function login() {
  ...
  try {
    ...
    if (j.ok) { sessionStorage.setItem('bobbyesquire_auth', '1'); checkAuth(); }
    else { loginError.style.display = 'block'; }
  } catch (e) {
    if (pw === ADMIN_PASSWORD) { sessionStorage.setItem('bobbyesquire_auth', '1'); checkAuth(); }
    else { loginError.style.display = 'block'; }
  }
}
...
async function recordSale(withBuyer) {
  ...
  if (_salePaidRaw !== '') {
    const _saleTotalNow = (Number(sale.salePrice) || 0) * (Number(sale.qty) || 1);
    sale.amountPaid = Math.min(_saleTotalNow, Math.max(0, parseInt(_salePaidRaw, 10) || 0));
  }
  closeSaleModal();
  try {
    let soldBag = null;
...
const TRASH_KEY = 'bobbyesquire_trash';
const TRASH_CAP = 50;
...
window.clientMessage = phone => {
  ...
  const msg = `Hi ${first}! Thanks for shopping with Bobby Esquire. Fresh pieces just landed. Browse what's new here: ${SHOP_URL}
...
function buildBroadcastMessage(recipientName) {
  ...
  return `${greet}It's Bobby Esquire. ${subject || 'Fresh stock just landed'}.${itemsBlock}\n\nTap to browse: ${SHOP_URL}
...
const BC_PROG_KEY = 'bobbyesquire_bcprog';
...
const INSIGHTS_KEY = 'bobbyesquire_analytics';
...
function posPrintReceipt() {
  ...
  document.getElementById('posReceiptPrint').innerHTML = `
    <div class="rcpt">
      <div class="rcpt-head">Bobby Esquire</div>
...
window.remindDebt = phone => {
  ...
  const intro = n === 1
    ? `A friendly reminder about your balance on the piece you took from Bobby Esquire:`
    : `A friendly reminder about the ${n} pieces you took from Bobby Esquire that still have a balance:`;
